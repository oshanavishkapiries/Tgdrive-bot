...
def getFilesByFolderId(service, folder_id):
    ...
    files = []
    while True:
        response = service.files().list(supportsAllDrives=True,
...
def copyFile(service, file_id, dest_id):
    ...
    try:
        return service.files().copy(supportsAllDrives=True, fileId=file_id, body=body).execute()
...
def create_directory(service, directory_name, parent_id):
    ...
    file = service.files().create(supportsAllDrives=True, body=file_metadata).execute()
    return file.get("id")
...
async def _copy(client, message):
    ...
    if meta.get("mimeType") == G_DRIVE_DIR_MIME_TYPE:
        ...
        try:
            _, transferred_size = cloneFolder(service, meta.get("name"), meta.get("name"), meta.get("id"), dir_id, transferred_size)
